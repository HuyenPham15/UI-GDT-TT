...
export function ThemKetQuaModal({ onClose, detail }: { onClose: () => void; detail?: any }) {
  ...
  const DON_LIST = [
    ...
    { id: "don-2", label: "Hoàng Anh, số TL: 123458, ngày TL: 04/05/2026", nguoi: ["Hoàng Anh", "Trần Văn Hùng"] },
    { id: "don-3", label: "Phạm Minh Tuấn, số TL: 123459, ngày TL: 05/05/2026", nguoi: ["Phạm Minh Tuấn"] },
  ];
  const [donOpen, setDonOpen] = useState(false);
  const [donExpanded, setDonExpanded] = useState<Record<string, boolean>>({});
  const [donChecked, setDonChecked] = useState<Record<string, boolean>>({ "don-1": true });
  ...
  const getSelectedLabel = () => {
    const parts: string[] = [];
    DON_LIST.forEach(d => {
      if (donChecked[d.id]) parts.push(d.label);
      else d.nguoi.forEach(n => { if (donChecked[`${d.id}::${n}`]) parts.push(`${n} (${d.label})`); });
    });
  ...
  // VKS and Trả lời đơn share nơi nhận; Kháng nghị and Xếp đơn have simpler defaults
  const showNoiNhan = isTraLoi || isVks || isKhangNghi;
      ...
      <div style={{ background: "#fff", borderRadius: 8, width: "min(900px, 96vw)", maxHeight: "92vh", display: "flex", flexDirection: "column", boxShadow: "0 8px 32px rgba(0,0,0,0.2)", fontFamily: F, overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px" }}>
          ...
          <div style={{ background: "#f0fdf4", border: `1px solid #bbf7d0`, borderRadius: 6, padding: "10px 16px", marginBottom: 16, display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "6px 16px", fontSize: 11 }}>
            <div><span style={{ color: MUTED }}>Mã vụ án: </span><span style={{ color: "#0f766e", fontWeight: 600 }}>VA26-001793</span></div>
            ...
            <div><span style={{ color: MUTED }}>Tên vụ án: </span><span style={{ color: "#0f766e", fontWeight: 600 }}>BIỆN TIỂU VY - BÙI LAN NHI</span></div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 13, fontWeight: 700, color: RED }}>Thông tin đơn</span>
          </div>
          <div style={{ marginBottom: 12, position: "relative" }}>
            <label style={lblSt}><span style={{ color: RED }}>*</span> Đơn liên quan</label>
            <div onClick={() => setDonOpen(o => !o)} style={{ ...inSt, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "space-between", userSelect: "none" }}>
            ...
            {donOpen && (
              <div style={{ position: "absolute", top: "100%", left: 0, right: 0, zIndex: 200, background: "#fff", border: `1px solid ${BORDER}`, borderRadius: 4, boxShadow: "0 4px 16px rgba(0,0,0,0.12)", maxHeight: 260, overflowY: "auto" }}>
                  ...
                  return (
                    <div key={don.id}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderBottom: `1px solid ${BORDER}`, background: isWholeDonChecked || anyNguoiChecked ? "#fef9f9" : "#fff" }}>
                        <input type="checkbox" checked={isWholeDonChecked}
                          ref={el => { if (el) el.indeterminate = !isWholeDonChecked && anyNguoiChecked; }}
                          onChange={() => toggleDon(don.id)} style={{ accentColor: RED, cursor: "pointer", flexShrink: 0 }} />
                        <span style={{ flex: 1, fontSize: 12, color: TEXT, wordBreak: "break-word" }}>{don.label}</span>
                        <button onClick={e => { e.stopPropagation(); setDonExpanded(p => ({ ...p, [don.id]: !p[don.id] })); }}
                      ...
                      {isExpanded && don.nguoi.map(nguoi => (
                        <div key={nguoi} style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 12px 6px 32px", borderBottom: `1px solid ${BORDER}`, background: donChecked[`${don.id}::${nguoi}`] ? "#fff5f5" : "#fafafa" }}>
                          <input type="checkbox" checked={!!donChecked[`${don.id}::${nguoi}`] || isWholeDonChecked}
                            onChange={() => toggleNguoi(don.id, nguoi)} style={{ accentColor: RED, cursor: "pointer", flexShrink: 0 }} />
                          <span style={{ fontSize: 12, color: TEXT }}>👤 {nguoi}</span>
                        </div>
                      ))}
                    </div>
                  );
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", padding: "8px 12px", borderTop: `1px solid ${BORDER}` }}>
                  <button onClick={() => setDonOpen(false)} style={{ padding: "5px 16px", background: RED, color: "#fff", border: "none", borderRadius: 4, cursor: "pointer", fontSize: 12, fontWeight: 600 }}>Xác nhận</button>
                </div>
              </div>
            )}
          </div>

          {/* Kết quả giải quyết radio */}
            ...
            <div style={{ display: "flex", gap: 20, alignItems: "center", flexWrap: "wrap" }}>
              {RADIO_OPTIONS.map(o => (
                <label key={o.value} style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, cursor: "pointer" }}>
                  <input type="radio" name="ketqua-tkq" checked={ketQua === o.value} onChange={() => { setKetQua(o.value); setAddingRow(false); }} style={{ accentColor: RED }} />
                  {o.label}
                </label>
              ))}
            </div>
          ...
          {(isTraLoi || isVks) && (
            <>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12, marginBottom: 12 }}>
                <div>
                  ...
                  <div style={{ position: "relative" }}>
                    <input type="text" placeholder="Chọn ngày" style={{ ...inSt, paddingRight: 30 }} />
              ...
              {isVks && (
                <div style={{ marginBottom: 16 }}>
                  <label style={lblSt}><span style={{ color: RED }}>*</span> Viện kiểm sát đang giải quyết</label>
                  <select style={inSt}>
                    <option value="">Chọn viện kiểm sát</option>
                    {VKS_OPTIONS.map(v => <option key={v}>{v}</option>)}
                  </select>
                </div>
              )}
            </>
            ...
            <>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 12 }}>
                <div>
                  ...
                  <select style={inSt}><option value="">Chọn thẩm quyền</option><option>Giám đốc thẩm</option><option>Tái thẩm</option></select>
                </div>
                <div>
                  <label style={lblSt}>Ngày phát hành</label>
          ...
          {isXepDon && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 16 }}>
              <div>
                <label style={lblSt}><span style={{ color: RED }}>*</span> Ngày xếp đơn</label>
                <div style={{ position: "relative" }}>
                  <input type="text" placeholder="Chọn ngày" style={{ ...inSt, paddingRight: 30 }} />
                  <span style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", fontSize: 13, color: MUTED, pointerEvents: "none" }}>📅</span>
                </div>
              ...
              <div>
                <label style={lblSt}><span style={{ color: RED }}>*</span> Người quyết định xếp đơn</label>
                <select style={inSt}>
                  <option value="">Chọn người quyết định</option>
                  <option>Nguyễn Hòa Bình – Chánh án</option>
                  <option>Lê Minh Trí – Phó Chánh án</option>
                  <option>Nguyễn Thị Bình – Vụ trưởng</option>
                </select>
              </div>
            </div>
          )}
          ...
          {isXepDon && (
            <div style={{ marginBottom: 16 }}>
              ...
              <textarea placeholder="Nhập lý do xếp đơn" style={{ ...inSt, minHeight: 80, resize: "vertical" }} />
            </div>
          )}

          {/* Nơi nhận – only for Trả lời đơn, Viện kiểm sát, Kháng nghị */}
          {showNoiNhan && (
            <>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed", marginBottom: 4 }}>
                ...
                <tbody>
                    ...
                    <tr key={r.id} style={{ background: idx % 2 === 0 ? "#fff" : "#fafafa" }}>
                      ...
                      <td style={{ ...TD_STYLE, textAlign: "center" }}>
                        <div style={{ display: "flex", gap: 8, justifyContent: "center", fontSize: 11 }}>
                          ...
                          <button onClick={() => setNnRows(p => p.filter(x => x.id !== r.id))} style={{ background: "none", border: "none", cursor: "pointer", color: "#ef4444", fontFamily: F, fontSize: 11 }}>🗑 Xóa</button>
                        </div>
                      </td>
                    </tr>
                  ))}
                  {addingRow && (
                    <tr>
                      <td style={{ ...TD_STYLE, textAlign: "center", fontSize: 11 }}>{nnRows.length + 1}</td>
                      ...
                      <td style={{ ...TD_STYLE, textAlign: "center" }}>
                        <div style={{ display: "flex", gap: 6, justifyContent: "center", fontSize: 11 }}>
                          <button onClick={() => { setNnRows(p => [...p, { id: Date.now(), noiNhan: "Nơi nhận", chiTiet: "Chi tiết", ghiChu: newGhiChu }]); setAddingRow(false); setNewGhiChu(""); }}
