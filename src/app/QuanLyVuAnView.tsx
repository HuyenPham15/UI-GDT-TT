...
export function renderThongTinBanAnCell(
  ...
  const dateLabel_DeNghi = label_DeNghi.replace("Số ", "Ngày ");
  const dateLabel_ConLai = label_ConLai.replace("Số ", "Ngày ");
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 5, lineHeight: 1.4, fontFamily: F }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: "2px 6px", fontSize: 11, fontWeight: 700, color: "#000000ff", fontFamily: F }}>
        <span>{label_DeNghi} {soBA_DeNghi}</span>
        {ngayBA_DeNghi && <span>– {dateLabel_DeNghi}: {ngayBA_DeNghi}</span>}
        <span>– Tại: {toa_DeNghi}</span>
      </div>

      {/* 2. Bản án Giai đoạn còn lại */}
      <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: "2px 6px", fontSize: 11, color: "#64748b", fontStyle: "italic", fontFamily: F, marginTop: 2, paddingTop: 3, borderTop: "1px dashed #e2e8f0" }}>
        <span>{label_ConLai} {soBA_ConLai}</span>
        {ngayBA_ConLai && <span>– {dateLabel_ConLai}: {ngayBA_ConLai}</span>}
        <span>– Tại: {toa_ConLai}</span>
      </div>
      {(row.nkn || row.biCao) && (
...
export default function QuanLyVuAnView({
  ...
  const getThongBaoTinhThe = (row: VuAnRow) => {
    return {
      soThongBao: `${125 + ((row.stt || 1) % 10)}/2026/TB-TANDTC-V1`,
      ngayThongBao: "14/08/2026",
      daKy: true,
    };
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden" }}>
                    ...
                    >
                      ...
                      <td colSpan={4} style={{ ...TD_STYLE, background: "#fef2f2", padding: "5px 10px" }}>
                        ...
                        >
                          ...
                          <span style={{ fontSize: 12, fontWeight: 700, color: "#1e293b", fontFamily: F }}>
                            {label_DeNghi}: {group.soBA || group.id}
                          </span>
                          <span style={{ fontSize: 12, fontWeight: 600, color: "#1e293b", fontFamily: F }}>
                            - {label_DeNghi.replace("Số ", "Ngày ")}: {group.ngayBA || group.id}
                          </span>
                          <span style={{ fontSize: 12, fontWeight: 600, color: "#1e293b", fontFamily: F }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'row', gap: 10, alignItems: 'center', marginTop: 4 }}>
                          <span style={{ fontSize: 11, fontWeight: 600, color: "#475569", fontFamily: F, display: "inline-flex", alignItems: "center" }}>
                            Tổng đơn đã thụ lý: <span style={{ color: "#475569", fontWeight: 700, marginLeft: 4 }}>{group.rows.length}</span>
                          </span>
                        </div>
                      </td>
                      ...
                      <td style={{ ...TD_STYLE, background: "#fef2f2", padding: "5px 8px" }}>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: 5, paddingTop: 4, marginTop: 2 }}>
                      ...
                      group.rows.map((row, idx) => {
                        ...
                        return (
                          ...
                          >
                                ...
                                <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                                  ...
                                  {row.anLoai === "quoc-hoi" && (
                                    <div style={{ fontSize: 10, color: "#475569", fontFamily: F, borderTop: "1px dashed #e2e8f0", paddingTop: 4, marginTop: 4, lineHeight: 1.3 }}>
                                      <span style={{ fontWeight: 600, color: "#3730a3" }}>Công văn:</span>{" "}
                                      <span>
                                        {row.stt % 3 === 0
                                          ? "Ban Dân nguyện"
                                          : row.stt % 3 === 1
                              ...
                              <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginTop: 2, alignItems: "center" }}>
                                ...
                                {row.anLoai === "quoc-hoi" && (
                                  <button
                                    onClick={(e) => {
                                      e.stopPropagation();
                                      setPreviewThongBaoTinhTheRow(row);
                                    }}
                                    style={{
                                      marginTop: 4,
                                      display: "inline-flex",
                                    ...
                                    title="Nhấn để mở popup xem biểu mẫu Thông báo tình thế (đã ký)"
                                  >
                                    <span>📄 TB tình thế:</span>
                                    <span style={{ color: "#1d4ed8", fontWeight: 700, textDecoration: "underline" }}>
                                      Số {getThongBaoTinhThe(row).soThongBao} ({getThongBaoTinhThe(row).ngayThongBao})
                                    </span>
                                    <Badge color="#065f46" bg="#d1fae5" style={{ fontSize: 9, padding: "1px 4px", flexShrink: 0 }}>✓ Đã ký</Badge>
                                  </button>
                                )}
                              </div>
                            </td>
                            ...
                            <td style={{ ...TD_STYLE, padding: "6px 8px" }}>
      ...
      {
        showTaoThongBaoTinhTheRow && (
          <TaoThongBaoTinhTheModal
            row={showTaoThongBaoTinhTheRow}
            onClose={() => setShowTaoThongBaoTinhTheRow(null)}
          />
        )
      }
      {
        previewThongBaoTinhTheRow && (
          <XemBieuMauThongBaoTinhTheModal
            row={previewThongBaoTinhTheRow}
            detail={previewThongBaoTinhTheRow}
            onClose={() => setPreviewThongBaoTinhTheRow(null)}
            soThongBao={getThongBaoTinhThe(previewThongBaoTinhTheRow).soThongBao}
            ngayThongBao={getThongBaoTinhThe(previewThongBaoTinhTheRow).ngayThongBao}
            coQuanNhan="Văn phòng Quốc hội - Ban Dân nguyện"
            nguoiKy="Nguyễn Thị Bình - Vụ trưởng"
            noiDung="Hiện nay, Thẩm tra viên được phân công đã rút toàn bộ hồ sơ vụ án từ Tòa án cấp dưới và đang tiến hành nghiên cứu, đối chiếu đánh giá chứng cứ. Tòa án nhân dân tối cao đã có văn bản thông báo tính thế tới Quý Cơ quan theo quy định."
          />
        )
      }
...
function ThongTinChungVuAnCard({ detail, onShowTaoThongBaoTinhThe, onShowPreviewThongBaoTinhThe }: { detail?: VuAnDetailData; onShowTaoThongBaoTinhThe?: (d: any) => void; onShowPreviewThongBaoTinhThe?: (d: any) => void }) {
  const detailTags: string[] = (detail as any)?.tags || [];
  const isAnQuocHoi = detail?.anLoai === "quoc-hoi" || detailTags.includes("an-quoc-hoi") || detailTags.includes("Án quốc hội") || (detail as any)?.anDacThu === "an-quoc-hoi" || (detail as any)?.anDacThu === "Án quốc hội";
  return (
      ...
      <div style={{ display: "flex", gap: 8, padding: "10px 16px 0", alignItems: "center", flexWrap: "wrap" }}>
        ...
        {isAnQuocHoi && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: "auto", flexWrap: "wrap" }}>
            <button
              onClick={() => onShowPreviewThongBaoTinhThe?.(detail)}
              style={{
                display: "inline-flex",
                alignItems: "center",
                gap: 5,
              ...
              title="Xem biểu mẫu Thông báo tình thế (đã ký)"
            >
              <span>Biểu mẫu TB tình thế:</span>
              <span style={{ color: "#1d4ed8", fontWeight: 700, textDecoration: "underline" }}>
                (Số 125/2026/TB-TANDTC – Ngày 14/08/2026)
              </span>
              <Badge color="#065f46" bg="#d1fae5" style={{ fontSize: 9, padding: "1px 5px" }}>✓ Đã ký</Badge>
            </button>
            <button
              onClick={() => onShowTaoThongBaoTinhThe?.(detail)}
              style={{
                display: "inline-flex",
                alignItems: "center",
                gap: 4,
              ...
              title="Tạo thông báo tình thế mới"
            >
              + Tạo mới
            </button>
          </div>
            ...
            return (
              <>
                <tr>
                  ...
                  <td style={{ ...TD_STYLE, background: BG, fontWeight: 600, fontSize: 11, color: MUTED, borderRight: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}` }}>Số {baCodeVuAn}</td>
                  <td style={{ ...TD_STYLE, fontSize: 12, color: TEXT, borderRight: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}` }}>{detail?.soBanAn || (detail?.soNgayBanAn ? detail.soNgayBanAn.split("–")[0]?.trim() : "12/4/2026/HSPT")}</td>
                  <td style={{ ...TD_STYLE, background: BG, fontWeight: 600, fontSize: 11, color: MUTED, borderRight: `1px solid ${BORDER}`, borderBottom: `1px solid ${BORDER}` }}>Ngày ra {baCodeVuAn}</td>
                  <td style={{ ...TD_STYLE, fontSize: 12, color: TEXT, borderBottom: `1px solid ${BORDER}` }}>{detail?.ngayBanAn || (detail?.soNgayBanAn ? detail.soNgayBanAn.split("–")[1]?.trim() : "30/12/2025")}</td>
                </tr>
...
function TabDanhSachDon({ detail, onShowTaoThongBaoTinhThe, onShowPreviewThongBaoTinhThe }: { detail: VuAnDetailData; onShowTaoThongBaoTinhThe?: (d: any) => void; onShowPreviewThongBaoTinhThe?: (d: any) => void }) {
      ...
      <div style={{ background: "#fff", borderRadius: 8, border: `1px solid ${BORDER}`, overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", padding: "12px 16px", borderBottom: `1px solid ${BORDER}` }}>
          ...
          {/* {isDetailAnQuocHoi && (
            <button
              onClick={() => onShowTaoThongBaoTinhThe?.(detail)}
              style={{ marginRight: 8, display: "flex", alignItems: "center", gap: 6, padding: "5px 12px", background: "#3730a3", color: "#fff", border: "none", borderRadius: 4, cursor: "pointer", fontSize: 12, fontWeight: 700, fontFamily: F }}
              title="Tạo thông báo tính thế cho vụ án Quốc hội"
            >
              + Tạo thông báo tình thế
            </button>
          )} */}
                ...
                <tbody>
                  ...
                  {addingRow && (
                    <tr style={{ background: "#f0f9ff" }}>
                      ...
                      <td style={{ ...TD_STYLE, textAlign: "center", fontSize: 11, color: MUTED }}>-</td>
                      <td style={{ ...TD_STYLE, textAlign: "center" }}>
                        <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
...
function ModalChonDonCanXuLy({
  ...
  const handleConfirm = () => {
    const selectedList = filteredList.filter(item => selectedIds.includes(item.id));
  ...
  const filteredList = sampleDonList.filter(item => {
    if (item.thongTinGQ !== "Thụ lý mới") return false;
      ...
      <div style={{ background: "#fff", borderRadius: 8, width: "100%", maxWidth: 780, boxShadow: "0 10px 40px rgba(0,0,0,0.25)", marginBottom: 24, overflow: "hidden", fontFamily: F }}>
        ...
        <div style={{ padding: "16px 20px 24px", display: "flex", flexDirection: "column", gap: 16, maxHeight: "82vh", overflowY: "auto" }}>
          <div style={{ border: "1px solid #e5e7eb", borderRadius: 6, padding: "14px 18px", background: "#fff" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1.1fr 1fr", gap: "10px 18px", fontSize: 12, fontFamily: F, lineHeight: 1.5 }}>
              {/* <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
...
function TabToTrinh({ detail, userRole, onShowTaoThongBaoTinhThe }: { detail?: VuAnDetailData; userRole?: UserRoleType; onShowTaoThongBaoTinhThe?: (d: any) => void }) {
  ...
  const [showTaoThongBaoTinhThe, setShowTaoThongBaoTinhThe] = useState(false);
  const [thuHoiIdx, setThuHoiIdx] = useState<number | null>(null);
  const [showTaoVanBanDropdown, setShowTaoVanBanDropdown] = useState(false);
  ...
  const [lichSuBatches, setLichSuBatches] = useState([
    {
      id: "batch-1",
      lanTrinh: "Lần 4",
      ngayTrinh: "13/08/2026",
      dsVanBan: [
        "Dự thảo Quyết định kháng nghị GDT",
      ],
      ngayDuyetCuoi: "–",
      trangThaiChung: "cho-duyet",
      dsLanhDao: [
        {
          lanhDao: "Lê Hoàng Nam - Chánh án",
          chucVu: "Chánh Án ",
          ngayDuyet: "–",
    ...
    },
    {
      ...
      dsVanBan: [
        "Tờ trình thẩm tra vụ việc ngày 09/08/2026",
        "Dự thảo Quyết định kháng nghị GĐT Nguyễn Văn Thành (Số TL: 28)",
        "Dự thảo Thông báo trả lời đơn Nguyễn Văn Thành (Số TL: 28)"
      ],
      ngayDuyetCuoi: "–",
      trangThaiChung: "cho-duyet",
      dsLanhDao: [
        {
          lanhDao: detail?.thamPhan || "Nguyễn Hạ Long",
          chucVu: "Phó Chánh án",
          ngayDuyet: "12/08/2026",
          trangThai: "da-duyet" as const,
          danhSachYKienTheoDon: [
            { donLabel: "Phạm Minh Tuấn - STL: 25 - 15/05/2026", mauYKien: "Kháng nghị" },
          ],
        ...
        },
        {
          isToThamPhan: true,
          lanhDao: [
            { lanhDao: "Nguyễn Văn A - Thẩm phán Tối cao", chucVu: "Thẩm phán Tối cao", ngayDuyet: "12/08/2026", trangThai: "da-duyet" as const, danhSachYKienTheoDon: [{ donLabel: "Phạm Minh Tuấn - STL: 25 - 15/05/2026", mauYKien: "Kháng nghị" }], yKienChungVanBan: " Đồng ý với quan điểm của Vụ trưởng Vụ 1 về việc kháng nghị." },
            { lanhDao: "Phan Hữu Nghị - Thẩm phán Tối cao", chucVu: "Thẩm phán Tối cao", ngayDuyet: "12/08/2026", trangThai: "da-duyet" as const, danhSachYKienTheoDon: [{ donLabel: "Phạm Minh Tuấn - STL: 25 - 15/05/2026", mauYKien: "Kháng nghị" }], yKienChungVanBan: "" },
            { lanhDao: "Vũ Quốc Hùng - Thẩm phán Tối cao", chucVu: "Thẩm phán Tối cao", ngayDuyet: "12/08/2026", trangThai: "da-duyet" as const, danhSachYKienTheoDon: [{ donLabel: "Phạm Minh Tuấn - STL: 25 - 15/05/2026", mauYKien: "Kháng nghị" }], yKienChungVanBan: "" }
          ],
          chucVu: "Tổ thẩm phán",
          trangThai: "da-duyet" as const,
          dsMembers: [
            {
              lanhDao: "Phan Hữu Nghị - Thẩm phán TAND Tối cao",
              chucVu: "Thẩm phán TAND Tối cao",
            ...
            {
              lanhDao: "Vũ Quốc Hùng - Thẩm phán TAND Tối cao",
              chucVu: "Thẩm phán TAND Tối cao",
              ngayDuyet: "10/08/2026",
              trangThai: "da-duyet" as const,
              danhSachYKienTheoDon: [],
    ...
    },
    {
      id: "batch-3",
      lanTrinh: "Lần 2",
      ngayTrinh: "10/08/2026",
      dsVanBan: [
        "Tờ trình thẩm tra vụ việc ngày 10/08/2026",
      ],
      ngayDuyetCuoi: "11/08/2026",
      trangThaiChung: "da-duyet",
      dsLanhDao: [
        {
          lanhDao: detail?.thamPhan || "Nguyễn Thị Bình - Vụ trưởng",
          chucVu: "Vụ trưởng",
          ngayDuyet: "11/08/2026",
          trangThai: "da-duyet" as const,
          danhSachYKienTheoDon: [
            { donLabel: "Phạm Minh Tuấn - STL: 25 - 15/05/2026", mauYKien: "Kháng nghị" },
            { donLabel: "Phùng Mạnh Hoàng - STL: 12 - 15/05/2026", mauYKien: "Nghiên cứu xác minh, bố sung" },
          ],
        ...
        {
          lanhDao: "Trần Văn B - Phó Vụ trưởng",
          chucVu: "Phó Vụ trưởng",
          ngayDuyet: "10/08/2026",
          trangThai: "da-duyet" as const,
          danhSachYKienTheoDon: [
            { donLabel: "Phạm Minh Tuấn - STL: 25 - 15/05/2026", mauYKien: "Trả lời đơn" },
            { donLabel: "Phùng Mạnh Hoàng - STL: 12/2026", mauYKien: "Trả lời đơn" },
          ],
        ...
        {
          lanhDao: "Phan Văn Nam - Thẩm phán TANDTC bậc 3",
          chucVu: "Thẩm phán TANDTC bậc 3",
          ngayDuyet: "10/08/2026",
          trangThai: "da-duyet" as const,
          danhSachYKienTheoDon: [
            { donLabel: "Phạm Minh Tuấn - STL: 25 - 15/05/2026", mauYKien: "Kháng nghị" },
            { donLabel: "Phùng Mạnh Hoàng - STL: 12 - 15/05/2026", mauYKien: "Trả lời đơn" },
          ],
  ...
  const formattedDonStr = `${nguoiStr} (Số TL: ${soTLStr} - ${ngayTLStr})`;

  const [vanBanList, setVanBanList] = useState(() => {
    const isKhieuNaiCase =
      detail?.isKhieuNai ||
      detail?.entityWord === "Khiếu nại" ||
      detail?.moduleLabel === "Quản lý khiếu nại" ||
      (typeof detail?.maVuAn === "string" && (detail.maVuAn.startsWith("KN") || detail.maVuAn.includes("KN"))) ||
    ...
    const items = [
      { stt: 1, loai: "to-trinh", vanBan: "Tờ trình thẩm tra vụ án ngày 09/08/2026", don: formattedDonStr, ngayTao: "09/08/2026", nguoiKy: detail?.thamTraVien || "Lý Thái Phúc (TTV)", trangThai: "Đã ký số", daDinhKemHoSo: true, soHoSo: 5 }
    ];

    if (isKhieuNaiCase) {
      items.push(
        { stt: 2, loai: "du-thao", vanBan: "Dự thảo Quyết định giải quyết khiếu nại (Chấp nhận khiếu nại)", don: formattedDonStr, ngayTao: "09/08/2026", nguoiKy: "–", trangThai: "Chờ ký số", daDinhKemHoSo: true, soHoSo: 3 },
        { stt: 3, loai: "du-thao", vanBan: "Dự thảo Quyết định giải quyết khiếu nại (Không chấp nhận khiếu nại)", don: formattedDonStr, ngayTao: "08/08/2026", nguoiKy: detail?.thamTraVien || "Lý Thái Phúc (TTV)", trangThai: "Đã ký số", daDinhKemHoSo: true, soHoSo: 1 }
      );
    } else {
      items.push(
        { stt: 2, loai: "du-thao", vanBan: "Dự thảo Quyết định kháng nghị giám đốc thẩm", don: formattedDonStr, ngayTao: "09/08/2026", nguoiKy: "–", trangThai: "Chờ ký số", daDinhKemHoSo: true, soHoSo: 3 },
        { stt: 3, loai: "du-thao", vanBan: "Dự thảo Thông báo trả lời đơn", don: formattedDonStr, ngayTao: "08/08/2026", nguoiKy: detail?.thamTraVien || "Lý Thái Phúc (TTV)", trangThai: "Đã ký số", daDinhKemHoSo: true, soHoSo: 1 }
      );
    }
    return items;
  });
  ...
  return (
    <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      {showTaoThongBaoTinhThe && (
        <TaoThongBaoTinhTheModal
          onClose={() => setShowTaoThongBaoTinhThe(false)}
          detail={detail}
          onSave={(data) => {
            const newRow = {
              stt: 1,
              loai: "du-thao",
              vanBan: "Thông báo tính thế tình hình giải quyết vụ án (Án Quốc hội)",
              don: formattedDonStr,
              ngayTao: data?.ngayThongBao || "14/08/2026",
              nguoiKy: data?.nguoiKy || "Nguyễn Thị Bình",
              trangThai: "Chờ ký số",
              daDinhKemHoSo: true,
              soHoSo: 2
            };
      ...
      <div style={{ background: "#fff", borderRadius: 8, border: `1px solid ${BORDER}`, overflow: "hidden" }}>
        <div style={{ display: "flex", alignItems: "center", padding: "12px 16px", borderBottom: `1px solid ${BORDER}`, gap: 8, flexWrap: "wrap" }}>
          ...
          >
            <button
              onClick={() => setShowTaoVanBanDropdown(v => !v)}
              style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 14px", background: "#fff", color: RED, border: `1px solid ${RED}`, borderRadius: 4, cursor: "pointer", fontSize: 12, fontWeight: 700, fontFamily: F }}
            >
              + Tạo văn bản <ChevronDown size={12} />
            </button>
            {showTaoVanBanDropdown && (
              <div
                style={{
                  position: "absolute",
                  top: "100%",
              ...
              >
                <div
                  onClick={() => {
                    setShowTaoTT(true);
                    setShowTaoVanBanDropdown(false);
                  }}
                  style={{
                    padding: "8px 12px",
                    fontSize: 12,
                ...
                <div
                  ...
                  onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}
                >
                  Tạo dự thảo kết quả giải quyết
                </div>
          ...
          {/* <button
            onClick={() => setShowTaoThongBaoTinhThe(true)}
            style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 14px", background: "#3730a3", color: "#fff", border: "none", borderRadius: 4, cursor: "pointer", fontSize: 12, fontWeight: 700, fontFamily: F }}
            title="Tạo thông báo tình thế cho vụ án Quốc hội"
          >
            + Tạo thông báo tình thế
          </button> */}
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed", minWidth: 600 }}>
            <colgroup>
              <col style={{ width: 40 }} />
              <col style={{ width: !isVu234(userRole, detail?.loaiAn) ? "28%" : "40%" }} />
              {!isVu234(userRole, detail?.loaiAn) && <col style={{ width: "16%" }} />}
              <col style={{ width: !isVu234(userRole, detail?.loaiAn) ? "10%" : "11%" }} />
              <col style={{ width: !isVu234(userRole, detail?.loaiAn) ? "12%" : "13%" }} />
              <col style={{ width: !isVu234(userRole, detail?.loaiAn) ? "11%" : "12%" }} />
              <col style={{ width: !isVu234(userRole, detail?.loaiAn) ? "13%" : "14%" }} />
              <col style={{ width: 90 }} />
            </colgroup>
            <thead>
              <tr>
                {(!isVu234(userRole, detail?.loaiAn)
                  ? ["STT", "TÊN VĂN BẢN", "ĐƠN ", "NGÀY TẠO", "NGƯỜI KÝ", "TRẠNG THÁI", "NGƯỜI THAO TÁC", "THAO TÁC"]
                  : ["STT", "TÊN VĂN BẢN", "NGÀY TẠO", "NGƯỜI KÝ", "TRẠNG THÁI", "NGƯỜI THAO TÁC", "THAO TÁC"]
                ).map(h => (
              ...
              {vanBanList.map((r, idx) => {
                ...
                return (
                  <tr key={r.stt} style={{ background: idx % 2 === 0 ? "#fff" : "#fafafa" }}>
                    ...
                    <td
                      style={{ ...TD, color: "#2563eb", fontWeight: 600, cursor: "pointer" }}
                      onClick={() => setPreviewDoc(r.vanBan)}
                    >
                      {r.vanBan}
                    </td>
                    {!isVu234(userRole, detail?.loaiAn) && (
                    ...
                    <td style={{ ...TD, fontSize: 11.5, color: TEXT, textAlign: "center" }}>{r.nguoiThaoTac || (idx % 2 === 0 ? "Nguyễn Thị Hương" : "Vũ Đức Thiện")}</td>
      ...
      <div style={{ background: "#fff", borderRadius: 8, border: `1px solid ${BORDER}`, overflow: "hidden" }}>
        ...
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", tableLayout: "fixed", minWidth: 850 }}>
            ...
            <tbody>
              {(() => {
                const flatSubmissions = filteredBatches.flatMap((batch) => {
                  return batch.dsLanhDao.map((ld, ldIdx) => ({
                    batchId: batch.id,
                    ngayTrinh: batch.ngayTrinh,
                    dsVanBan: batch.dsVanBan,
                    trangThaiChung: batch.trangThaiChung,
                    ld: ld,
                    ldIdx: ldIdx,
                    rawBatch: batch
                  }));
                ...
                return flatSubmissions.flatMap((sub, subIdx) => {
                  const batch = sub.rawBatch;
                  const ld = sub.ld;

                  let displayDocs = sub.dsVanBan;
                  if (sub.batchId === "batch-3" && displayDocs.length === 1) {
                    displayDocs = [
                      "Tờ trình thẩm tra vụ việc ngày 09/08/2026",
                      "Dự thảo Quyết định kháng nghị GĐT Nguyễn Văn Thành (Số TL: 28)",
                      "Dự thảo Thông báo trả lời đơn Nguyễn Văn Thành (Số TL: 28)"
                    ];
                  } else if (sub.batchId === "batch-1") {
                    displayDocs = ["Dự thảo Quyết định kháng nghị GDT"];
                  } else if (sub.batchId === "batch-4") {
                    displayDocs = ["Dự thảo Quyết định kháng nghị GDT"];
                  }

                  const rowCount = ld.isToThamPhan && Array.isArray(ld.lanhDao) ? ld.lanhDao.length : 1;
                  const previousLeadersPending = batch.dsLanhDao
                    .slice(0, sub.ldIdx)
                    .some(prevLd => prevLd.trangThai === "cho-y-kien");

                  const getLdNames = (l: any) => {
                    if (!l) return "";
                    if (Array.isArray(l.lanhDao)) {
                      return l.lanhDao.map((x: any) => x.lanhDao.split(" - ")[0]).join(", ");
                    }
                    return typeof l.lanhDao === "string" ? l.lanhDao : "";
                  };

                  const isBlocked = ld.trangThai === "cho-y-kien" || previousLeadersPending;
                  const blockMessage = previousLeadersPending
                    ? `Lãnh đạo cấp dưới (${getLdNames(batch.dsLanhDao[sub.ldIdx - 1])}) chưa cho ý kiến`
                    : `Lãnh đạo ${getLdNames(ld)} chưa cho ý kiến`;

                  return Array.from({ length: rowCount }).map((_, dIdx) => {
                    const isLastRow = dIdx === rowCount - 1;

                    return (
                      <tr
                        key={`${sub.batchId}-${typeof ld.lanhDao === "string" ? ld.lanhDao : "to-tp"}-${dIdx}`}
                        style={{
                          background: subIdx % 2 === 0 ? "#fff" : "#fafafa",
                          borderBottom: isLastRow ? `1px solid ${BORDER}` : `1px dashed #e2e8f0`,
                        }}
                      >
                        {/* STT */}
                        {dIdx === 0 && (
                        ...
                        {dIdx === 0 && (
                          <td rowSpan={rowCount} style={{ ...TD, fontWeight: 600, verticalAlign: "middle" }}>
                            <div style={{ color: TEXT }}>{sub.ngayTrinh}</div>
                          </td>
                        )}

                        {/* VĂN BẢN */}
                        {dIdx === 0 && (
                          <td rowSpan={rowCount} style={{ ...TD, color: "#334155", verticalAlign: "top" }}>
                            {(() => {
                              const toTrinh = displayDocs.find(d => d.includes("Tờ trình"));
                              const duThaos = displayDocs.filter(d => d.includes("Dự thảo"));
                              ...
                              return (
                                <div>
                                  {toTrinh ? (
                                    <>
                                      <div
                                        style={{ fontWeight: 600, color: "#2563eb", marginBottom: duThaos.length > 0 ? 6 : 0, cursor: "pointer" }}
                                        onClick={() => setPreviewDoc(toTrinh)}
                                      >
                                        {toTrinh}
                                      </div>
                                      {duThaos.length > 0 && (
                                        <div style={{ paddingLeft: 12, borderLeft: "3px solid #cbd5e1", marginTop: 4 }}>
                                          <div style={{ fontSize: 11, color: MUTED, fontWeight: 600, marginBottom: 4 }}>Văn bản kèm theo:</div>
                                          <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                                            {duThaos.map((dt, dtIdx) => (
                                              <div
                                                key={dtIdx}
                                                style={{ fontSize: 11.5, color: "#2563eb", cursor: "pointer", fontWeight: 500 }}
                                                onClick={() => setPreviewDoc(dt)}
                                              >
                                                • {dt}
                                              </div>
                                            ))}
                        ...
                        {ld.isToThamPhan ? (
                          <td style={{ ...TD, verticalAlign: "middle" }}>
                            {Array.isArray(ld.lanhDao) && ld.lanhDao[dIdx] && (() => {
                              const item = ld.lanhDao[dIdx];
                              return (
                                <div style={{ fontSize: 12, color: "#1e293b" }}>
                                  <span style={{ fontWeight: 700 }}>{item.lanhDao}</span> - <span style={{ color: MUTED }}>({item.chucVu})</span>
                                </div>
                              );
                            })()}
                          </td>
                          ...
                          dIdx === 0 && (
                            <td rowSpan={rowCount} style={{ ...TD, verticalAlign: "top" }}>
                              <div>
                                <div style={{ fontWeight: 700, color: "#1e293b", fontSize: 12 }}>{typeof ld.lanhDao === "string" ? ld.lanhDao : ""}</div>
                        ...
                        {ld.isToThamPhan ? (
                          <td style={{ ...TD, verticalAlign: "middle" }}>
                            {Array.isArray(ld.lanhDao) && ld.lanhDao[dIdx] && (() => {
                              ...
                              return (
                                <div>
                                  {item.danhSachYKienTheoDon && item.danhSachYKienTheoDon.length > 0 && (
                                    <div style={{ display: "flex", flexDirection: "column", gap: 4, marginBottom: item.yKienChungVanBan ? 6 : 0 }}>
                                      {item.danhSachYKienTheoDon.map((yk, ykIdx) => (
                                        <div key={ykIdx} style={{ fontSize: 11.5 }}>
                                          <span style={{ fontWeight: 700, color: "#0f766e" }}>{yk.donLabel}: </span>
                                          <span
                                            style={{
                                              display: "inline-flex",
                                              alignItems: "center",
                          ...
                          dIdx === 0 && (
                            <td rowSpan={rowCount} style={{ ...TD, verticalAlign: "top" }}>
                              {isVu234(userRole, detail?.loaiAn) ? (
                                /* Dành cho Vụ 2, 3, 4: Ý kiến theo văn bản / vụ án */
                                <div>
                        ...
                        {/* NGÀY DUYỆT */}
                        {dIdx === 0 && (
                          <td rowSpan={rowCount} style={{ ...TD, verticalAlign: "top", padding: ld.isToThamPhan ? 0 : TD.padding, height: ld.isToThamPhan ? "1px" : "auto" }}>
                            {ld.isToThamPhan && Array.isArray(ld.lanhDao) ? (
                              <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
                                {ld.lanhDao.map((item, lIdx) => (
                                  <div key={lIdx} style={{ flex: 1, padding: "10px 12px", display: "flex", flexDirection: "column", justifyContent: "center", borderBottom: lIdx !== ld.lanhDao.length - 1 ? `1px solid ${BORDER}` : "none" }}>
                                    <div style={{ fontWeight: 600, color: item.ngayDuyet && item.ngayDuyet !== "–" ? TEXT : MUTED, fontSize: 12 }}>
                                      {item.ngayDuyet || "–"}
                                    </div>
                                    <div style={{ fontSize: 10.5, color: MUTED, marginTop: 2 }}>
                                      {item.trangThai === "da-duyet" ? (
                                        "Đã duyệt"
                                      ) : item.trangThai === "tu-choi" ? (
                                        "Từ chối"
                                      ) : (
                                        "Chưa duyệt"
                                      )}
                              ...
                              <div>
                                ...
                                <div style={{ fontSize: 10.5, color: MUTED, marginTop: 2 }}>
                                  {ld.trangThai === "da-duyet" ? (
                                    "Đã duyệt"
                                  ) : ld.trangThai === "tu-choi" ? (
                                    "Từ chối"
                                  ) : previousLeadersPending ? (
                                    "Chờ cấp dưới duyệt"
                                  ) : (
                                    "Chưa duyệt"
                                  )}
                          ...
                          <td rowSpan={rowCount} style={{ ...TD, textAlign: "center", verticalAlign: "middle" }}>
                            <div style={{ display: "flex", gap: 6, alignItems: "center", justifyContent: "center" }}>
                              <button
                                ...
                                title="Xem"
                                onClick={() => setPreviewDoc(displayDocs[0] || "Tờ trình thẩm tra")}
                              >
                              ...
                              {batch.trangThaiChung === "cho-duyet" && (
                                <button
                                  title="Thu hồi"
                                  onClick={() => {
                                    const originalIdx = lichSuBatches.findIndex(b => b.id === batch.id);
                                    if (originalIdx !== -1) setThuHoiIdx(originalIdx);
                                  }}
                                  style={{ background: "none", border: "none", cursor: "pointer", padding: 2 }}
                                >
                              ...
                              {batch.trangThaiChung === "tu-choi" ? (
                                <button
                                  title={isBlocked ? `Không thể trình ký do ${blockMessage}` : "Trình lại tờ trình"}
                                  onClick={() => {
                                    if (previousLeadersPending) {
                                      alert(`⚠️ Lãnh đạo cấp dưới (${getLdNames(batch.dsLanhDao[sub.ldIdx - 1])}) chưa cho ý kiến, không thể trình tiếp!`);
                                      return;
                                    }
                                    if (ld.trangThai === "cho-y-kien") {
                                      alert(`⚠️ Lãnh đạo ${getLdNames(ld)} chưa cho ý kiến, không được trình cấp tiếp theo!`);
                                      return;
                                    }
                                    setShowTrinhKy(true);
                                  }}
                                ...
                                <button
                                  title={isBlocked ? `Không thể trình ký do ${blockMessage}` : "Trình ký"}
                                  onClick={() => {
                                    if (previousLeadersPending) {
                                      alert(`⚠️ Lãnh đạo cấp dưới (${getLdNames(batch.dsLanhDao[sub.ldIdx - 1])}) chưa cho ý kiến, không thể trình tiếp!`);
                                      return;
      ...
      <div style={{ background: "#fff", borderRadius: 8, border: `1px solid ${BORDER}`, padding: "16px 20px", boxShadow: "0 1px 3px rgba(0,0,0,0.05)", marginTop: isKhieuNai ? 0 : 16 }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
          <span style={{ fontSize: 15, fontWeight: 700, color: "#111827", fontFamily: F }}>
            {isKhieuNai ? "Kết quả giải quyết" : "Kết quả giải quyết"}
          </span>
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, fontFamily: F }}>
                      <thead>
                        <tr style={{ background: "#fff", borderBottom: "1px solid #e5e7eb" }}>
                          ...
                          <th style={{ ...thSt, width: 140 }}>Số</th>
                          <th style={{ ...thSt, width: 120 }}>Ngày</th>
...
// ── Modal xem & tạo Thông báo tính thế cho Án Quốc hội ───────────────────────
export function XemBieuMauThongBaoTinhTheModal({
  onClose,
  detail,
  soThongBao,
  ngayThongBao,
  ...
  const today = new Date();
  const todayStr = ngayThongBao || `ngày ${today.getDate()} tháng ${today.getMonth() + 1} năm ${today.getFullYear()}`;
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#f1f5f9", zIndex: 3600, display: "flex", flexDirection: "column", width: "100vw", height: "100vh", overflow: "hidden", fontFamily: F }}>
      {/* Header ribbon Word style */}
      <div style={{ background: "#3730a3", color: "#fff", padding: "10px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0, boxShadow: "0 2px 8px rgba(0,0,0,0.15)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <button onClick={onClose} style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 14px", background: "rgba(255,255,255,0.15)", color: "#fff", border: "1px solid rgba(255,255,255,0.3)", borderRadius: 4, cursor: "pointer", fontSize: 12, fontWeight: 600, fontFamily: F }}>
            ← Quay lại
          </button>
          ...
          <div>
            <div style={{ fontWeight: 700, fontSize: 15, fontFamily: F, display: "flex", alignItems: "center", gap: 8 }}>
              <span>Thông báo tính thế tình hình giải quyết vụ án (Án Quốc hội).docx</span>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button onClick={() => alert("Đã lưu nội dung Thông báo tính thế Word thành công!")} style={{ padding: "7px 20px", background: "#15803d", color: "#fff", border: "none", borderRadius: 4, cursor: "pointer", fontSize: 13, fontWeight: 700, fontFamily: F }}>
            💾 Lưu thay đổi
          </button>
          <button onClick={onClose} style={{ background: "none", border: "none", cursor: "pointer", color: "#fff", padding: 4 }}>
            <X size={22} />
      ...
      <div style={{ flex: 1, overflow: "auto", padding: "30px 20px 60px 20px", display: "flex", justifyContent: "center", background: "#cbd5e1" }}>
        ...
        >
            ...
            <tbody>
              <tr>
                <td style={{ width: "45%", textAlign: "center", verticalAlign: "top" }}>
                  ...
                  <div style={{ width: 90, height: 1, background: "#000", margin: "4px auto" }} />
                  <div style={{ fontSize: "12pt", marginTop: 4 }}>
                    Số: <b>{soThongBao || "125/2026/TB-TANDTC-V1"}</b>
                  </div>
                </td>
                <td style={{ width: "55%", textAlign: "center", verticalAlign: "top" }}>
                  <div style={{ fontWeight: "bold", fontSize: "12pt" }}>CỘNG HÒA XÃ HỘI CHỦ NGHĨA VIỆT NAM</div>
          ...
          <div style={{ textAlign: "center", margin: "24px 0 14px" }}>
            ...
            <div style={{ fontSize: "13pt", fontWeight: "bold", marginTop: 4 }}>
              Về tình hình, diễn biến giải quyết vụ án (Án Quốc hội theo dõi/chuyển đơn)
            </div>
            <div style={{ fontSize: "12pt", fontStyle: "italic", marginTop: 2, color: "#374151" }}>
              (Theo Công văn chuyển đơn số 524/VPQH của {coQuanNhan || "Văn phòng Quốc hội"})
            </div>
          </div>

          <div style={{ textAlign: "center", fontSize: "13pt", fontWeight: "bold", marginBottom: 20 }}>
            Kính gửi: {coQuanNhan || "Văn phòng Quốc hội - Ban Dân nguyện Quốc hội"}
          </div>

          <div style={{ textAlign: "justify", lineHeight: 1.65 }}>
            <p style={{ margin: "4px 0", textIndent: "1cm" }}>
              Tòa án nhân dân tối cao nhận được Công văn của quý Cơ quan về việc chuyển đơn đề nghị xem xét theo thủ tục giám đốc thẩm/tái thẩm đối với vụ án <b>{detail?.tenVuAn || detail?.soBA || "Nguyễn Văn Minh – Tội cướp tài sản"}</b>.
            </p>
            <p style={{ fontWeight: "bold", margin: "14px 0 4px" }}>I. ĐÓNG GÓP TÌNH THẾ VÀ DIỄN BIẾN HIỆN TẠI CỦA VỤ ÁN:</p>
            <p style={{ margin: "4px 0", textIndent: "1cm" }}>
              {noiDung || "Hiện nay, Thẩm tra viên được phân công đã rút toàn bộ hồ sơ vụ án từ Tòa án cấp dưới và đang tiến hành nghiên cứu, đối chiếu đánh giá chứng cứ. Tòa án nhân dân tối cao đã yêu cầu Viện kiểm sát nhân dân tối cao phối hợp cung cấp các hồ sơ liên quan để xem xét căn cứ kháng nghị theo quy định pháp luật tố tụng."}
            </p>
            <p style={{ fontWeight: "bold", margin: "14px 0 4px" }}>II. DỰ KIẾN HƯỚNG XỬ LÝ TIẾP THEO:</p>
            <p style={{ margin: "4px 0", textIndent: "1cm" }}>
              Sau khi hoàn thành báo cáo thẩm tra tổng hợp, Vụ Giám đốc kiểm tra I sẽ lập Tờ trình báo cáo Lãnh đạo Vụ và trình Phó Chánh án Tòa án nhân dân tối cao xem xét quyết định theo thẩm quyền. Tòa án nhân dân tối cao sẽ có văn bản thông báo chính thức kết quả giải quyết tới Quý Cơ quan ngay khi có quyết định.
            </p>
            <p style={{ fontStyle: "italic", margin: "14px 0 24px" }}>
              Tòa án nhân dân tối cao trân trọng thông báo tính thế diễn biến giải quyết vụ án để Quý Cơ quan được biết và phối hợp./.
            </p>
          </div>
              ...
              <tr>
                ...
                <td style={{ width: "55%", textAlign: "center", verticalAlign: "top", fontSize: "12pt" }}>
                  ...
                  <div style={{ height: 75, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <div style={{ border: "2px dashed #166534", color: "#166534", padding: "4px 12px", borderRadius: 4, fontSize: "10pt", fontWeight: "bold" }}>
                      ✓ ĐÃ KÝ SỐ CHIẾU THEO TÍNH THẾ
                    </div>
                  </div>
                  <div style={{ fontWeight: "bold" }}>{nguoiKy || "Nguyễn Thị Bình"}</div>
                </td>
              </tr>
            </tbody>
          </table>
...
export function TaoThongBaoTinhTheModal({
  onClose,
  row,
  detail,
  onSave,
  ...
  const [ngayThongBao, setNgayThongBao] = useState("14/08/2026");
  const [soThongBao, setSoThongBao] = useState("125/2026/TB-TANDTC-V1");
  const [daLaySo, setDaLaySo] = useState(true);
  const [coQuanNhan, setCoQuanNhan] = useState("Văn phòng Quốc hội");
  const [nguoiKy, setNguoiKy] = useState("Nguyễn Thị Bình - Vụ trưởng");
  const [noiDung, setNoiDung] = useState(
    "Hiện nay, Thẩm tra viên được phân công đã rút toàn bộ hồ sơ vụ án từ Tòa án cấp dưới và đang tiến hành nghiên cứu, đối chiếu đánh giá chứng cứ. Tòa án nhân dân tối cao đang khẩn trương tập hợp tình hình để trình Chánh án TANDTC xem xét xử lý."
  );
  const [showBieuMau, setShowBieuMau] = useState(false);
  const [showTrinhKy, setShowTrinhKy] = useState(false);
  ...
  const handleToggleLaySo = () => {
    if (!daLaySo) {
      ...
      const generated = `${num}/2026/TB-TANDTC-V1`;
      setSoThongBao(generated);
      setDaLaySo(true);
    } else {
      setSoThongBao("");
      setDaLaySo(false);
  ...
  const handleSave = () => {
    ...
    alert("Đã lưu Thông báo tính thế thành công!");
    onClose();
  };

  const maVuAn = row?.soThuLy || detail?.maVuAn || "VA26-00321";
  const tenVuAn = detail?.tenVuAn || row?.biCao || "Vụ án Nguyễn Văn Minh – Tội cướp tài sản";
  const soBA = row?.soBA || detail?.soNgayBanAn || "124/2025/HSPT";

  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", zIndex: 3400, display: "flex", alignItems: "flex-start", justifyContent: "center", overflowY: "auto", padding: "24px 16px", fontFamily: F }}>
      {showTrinhKy && <TrinhKyModal onClose={() => setShowTrinhKy(false)} />}
      {showBieuMau && (
        <XemBieuMauThongBaoTinhTheModal
          onClose={() => setShowBieuMau(false)}
          ...
          coQuanNhan={coQuanNhan}
          nguoiKy={nguoiKy}
          noiDung={noiDung}
        />
      )}

      <div style={{ background: "#fff", borderRadius: 8, width: "100%", maxWidth: 780, boxShadow: "0 10px 40px rgba(0,0,0,0.25)", marginBottom: 24, overflow: "hidden" }}>
        {/* Header */}
        <div style={{ display: "flex", alignItems: "center", padding: "14px 20px", borderBottom: "1px solid #e5e7eb", background: "#e0e7ff" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, flex: 1 }}>
            <span style={{ fontSize: 18, color: "#3730a3" }}>🏛️</span>
            <span style={{ fontSize: 16, fontWeight: 700, color: "#3730a3", fontFamily: F }}>
              Tạo Thông báo tình thế (Án Quốc hội)
            </span>
          </div>
        ...
        {/* Body */}
        <div style={{ padding: "16px 20px 20px", display: "flex", flexDirection: "column", gap: 16, maxHeight: "82vh", overflowY: "auto" }}>
          {/* Info Card */}
          <div style={{ background: "#f8fafc", border: "1px solid #c7d2fe", borderRadius: 6, padding: "12px 16px" }}>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, fontSize: 12, fontFamily: F }}>
              <div><span style={{ color: MUTED }}>Thuộc vụ án: </span><b style={{ color: "#3730a3" }}>{maVuAn} - {tenVuAn}</b></div>
              <div><span style={{ color: MUTED }}>Số BA/QĐ: </span><b>{soBA}</b></div>
              <div><span style={{ color: MUTED }}>Đơn vị gửi công văn QH: </span><b style={{ color: "#1e40af" }}>Văn phòng Quốc hội / Ban Dân nguyện</b></div>
              <div><span style={{ color: MUTED }}>Phân loại: </span><Badge color="#3730a3" bg="#e0e7ff">Án Quốc hội</Badge></div>
            </div>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div>
              <label style={{ fontSize: 12, color: TEXT, fontWeight: 600, display: "block", marginBottom: 4 }}>Số thông báo</label>
              <input type="text" value={soThongBao} onChange={e => setSoThongBao(e.target.value)} style={{ width: "100%", padding: "7px 10px", fontSize: 12, border: `1px solid ${BORDER}`, borderRadius: 4, boxSizing: "border-box" }} />
            </div>
            <div>
              <label style={{ fontSize: 12, color: TEXT, fontWeight: 600, display: "block", marginBottom: 4 }}>Ngày lập thông báo</label>
              <input type="text" value={ngayThongBao} onChange={e => setNgayThongBao(e.target.value)} style={{ width: "100%", padding: "7px 10px", fontSize: 12, border: `1px solid ${BORDER}`, borderRadius: 4, boxSizing: "border-box" }} />
            </div>
          </div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div>
              <label style={{ fontSize: 12, color: TEXT, fontWeight: 600, display: "block", marginBottom: 4 }}>Cơ quan Quốc hội nhận thông báo</label>
              <select value={coQuanNhan} onChange={e => setCoQuanNhan(e.target.value)} style={{ width: "100%", padding: "7px 10px", fontSize: 12, border: `1px solid ${BORDER}`, borderRadius: 4, background: "#fff" }}>
                <option value="Văn phòng Quốc hội">Văn phòng Quốc hội</option>
                <option value="Ban Dân nguyện - Quốc hội">Ban Dân nguyện - Quốc hội</option>
                <option value="Ủy ban Tư pháp của Quốc hội">Ủy ban Tư pháp của Quốc hội</option>
                <option value="Đoàn Đại biểu Quốc hội">Đoàn Đại biểu Quốc hội</option>
              </select>
            </div>
            <div>
              <label style={{ fontSize: 12, color: TEXT, fontWeight: 600, display: "block", marginBottom: 4 }}>Người ký thông báo</label>
              <select value={nguoiKy} onChange={e => setNguoiKy(e.target.value)} style={{ width: "100%", padding: "7px 10px", fontSize: 12, border: `1px solid ${BORDER}`, borderRadius: 4, background: "#fff" }}>
                <option value="Nguyễn Thị Bình - Vụ trưởng">Nguyễn Thị Bình - Vụ trưởng</option>
                <option value="Nguyễn Văn Quảng - Phó Chánh án TANDTC">Nguyễn Văn Quảng - Phó Chánh án TANDTC</option>
          ...
          <div>
            <label style={{ fontSize: 12, color: TEXT, fontWeight: 600, display: "block", marginBottom: 4 }}>Nội dung thông báo tính thế / Diễn biến hiện tại</label>
            <textarea value={noiDung} onChange={e => setNoiDung(e.target.value)} rows={5} style={{ width: "100%", padding: "8px 10px", fontSize: 12, border: `1px solid ${BORDER}`, borderRadius: 4, resize: "vertical", boxSizing: "border-box", fontFamily: F, lineHeight: 1.5 }} />
          </div>
        </div>

        {/* Footer */}
        <div style={{ display: "flex", justifyContent: "center", gap: 10, padding: "14px 20px", borderTop: `1px solid ${BORDER}`, background: "#fff" }}>
          <button type="button" onClick={onClose} style={{ padding: "7px 20px", background: "#fff", color: TEXT, border: `1px solid ${BORDER}`, borderRadius: 4, cursor: "pointer", fontSize: 12, fontFamily: F }}>Đóng</button>
          <button type="button" onClick={handleSave} style={{ padding: "7px 24px", background: "#3730a3", color: "#fff", border: "none", borderRadius: 4, cursor: "pointer", fontSize: 12, fontWeight: 700, fontFamily: F }}>Lưu</button>
          <button type="button" onClick={handleToggleLaySo} style={{ padding: "7px 18px", background: daLaySo ? "#e0e7ff" : "#fff", color: daLaySo ? "#3730a3" : TEXT, border: `1px solid ${daLaySo ? "#c7d2fe" : BORDER}`, borderRadius: 4, cursor: "pointer", fontSize: 12, fontFamily: F }}>
            {daLaySo ? "Hủy số" : "Lấy số"}
          </button>
          <button type="button" onClick={() => setShowTrinhKy(true)} style={{ padding: "7px 24px", background: RED, color: "#fff", border: "none", borderRadius: 4, cursor: "pointer", fontSize: 12, fontWeight: 700, fontFamily: F }}>Trình ký</button>
          <button type="button" onClick={() => setShowBieuMau(true)} style={{ padding: "7px 20px", background: "#fff", color: TEXT, border: `1px solid ${BORDER}`, borderRadius: 4, cursor: "pointer", fontSize: 12, fontFamily: F }}>Xem biểu mẫu</button>
        </div>
      </div>
    </div>
  );
...
export function ChiTietVuAnView({
  ...
  const [showTaoThongBaoTinhTheDetail, setShowTaoThongBaoTinhTheDetail] = useState<any>(null);
  const [showPreviewThongBaoTinhTheDetail, setShowPreviewThongBaoTinhTheDetail] = useState<any>(null);
  ...
  const tabs: Array<{ id: ChiTietTab; label: string; count?: number }> = [
    ...
    { id: "giai-quyet-vb", label: "Kết quả giải quyết", count: 3 },
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%", background: "#f8fafc", overflow: "hidden", fontFamily: F }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 20px", borderBottom: `1px solid ${BORDER}`, background: "#fff", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {(detail.loaiAn === "quoc-hoi" || (detail as any)?.tags?.includes("an-quoc-hoi") || (detail as any)?.tags?.includes("Án quốc hội") || (detail as any)?.anDacThu === "an-quoc-hoi") && (
            <div style={{ display: "inline-flex", alignItems: "center", gap: 6, marginLeft: 8, flexWrap: "wrap" }}>
              <button
                onClick={() => setShowPreviewThongBaoTinhTheDetail(detail)}
                style={{
                  display: "inline-flex",
                  alignItems: "center",
                  gap: 4,
              ...
              >
                <span>📄 Biểu mẫu TB tình thế:</span>
                <span style={{ color: "#1d4ed8", fontWeight: 700, textDecoration: "underline" }}>
                  Số 125/2026/TB-TANDTC (14/08/2026)
                </span>
                <Badge color="#065f46" bg="#d1fae5" style={{ fontSize: 9, padding: "1px 5px" }}>✓ Đã ký</Badge>
              </button>
              <button
                onClick={() => setShowTaoThongBaoTinhTheDetail(detail)}
                style={{
                  display: "inline-flex",
                  alignItems: "center",
                  gap: 4,
                ...
                title="Tạo mới hoặc cập nhật Thông báo tính thế"
              >
                + Tạo mới
              </button>
            </div>
      ...
      <div style={{ flex: 1, overflowY: "auto" }}>
        ...
        {activeTab === "danh-sach-don" && <TabDanhSachDon detail={detail} onShowTaoThongBaoTinhThe={(d) => setShowTaoThongBaoTinhTheDetail(d)} onShowPreviewThongBaoTinhThe={(d) => setShowPreviewThongBaoTinhTheDetail(d)} />}
        {activeTab === "phan-cong" && <TabPhanCong detail={detail} />}
        {activeTab === "muon-tra-ho-so" && <TabMuonTraHoSo detail={detail} />}
        {activeTab === "to-trinh" && <TabToTrinh detail={detail} userRole={userRole} onShowTaoThongBaoTinhThe={(d) => setShowTaoThongBaoTinhTheDetail(d)} />}
      ...
      {showTaoThongBaoTinhTheDetail && (
        <TaoThongBaoTinhTheModal
          detail={showTaoThongBaoTinhTheDetail}
          onClose={() => setShowTaoThongBaoTinhTheDetail(null)}
        />
      )}
      {showPreviewThongBaoTinhTheDetail && (
        <XemBieuMauThongBaoTinhTheModal
          detail={showPreviewThongBaoTinhTheDetail}
          onClose={() => setShowPreviewThongBaoTinhTheDetail(null)}
          soThongBao="125/2026/TB-TANDTC-V1"
          ngayThongBao="14/08/2026"
          coQuanNhan="Văn phòng Quốc hội - Ban Dân nguyện"
          nguoiKy="Nguyễn Thị Bình - Vụ trưởng"
          noiDung="Hiện nay, Thẩm tra viên được phân công đã rút toàn bộ hồ sơ vụ án từ Tòa án cấp dưới và đang tiến hành nghiên cứu, đối chiếu đánh giá chứng cứ. Tòa án nhân dân tối cao đã có văn bản thông báo tính thế tới Quý Cơ quan theo quy định."
