...
|--------|--------------|-------------|
| List – Đơn chờ phê duyệt | `ChưaCoVụAn/index.tsx` | Tab "Đơn chờ phê duyệt (5)" active |
| List – Chờ xin ý kiến | `ChờXinYKiến/index.tsx` | Tab "Chờ xin ý kiến (4)" active |
...
### State
```ts
type View = 'don-cho-phe-duyet' | 'cho-y-kien' | 'da-co-vu-an' | 'ho-so-khang-nghi' | 'giao-tieu-ho-so' | 'them-ho-so'
const [view, setView] = useState<View>('don-cho-phe-duyet')
```
...
## Verification
1. App renders the Đơn chờ phê duyệt list by default
